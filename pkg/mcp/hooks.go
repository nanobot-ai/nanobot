...
func InvokeHooks[T any](ctx context.Context, r HookRunner, hooks Hooks, in *T, name string, params map[string]string) (T, error) {
	var (
		out     T
		current = in
		matched bool
	...
	for _, mapping := range hooks {
		if mapping.Matches(name, params) {
			for _, target := range mapping.Targets {
				...
				if err != nil {
					return out, fmt.Errorf("failed to run hook %s: %w", mapping.String(), err)
				}
				if hasOutput {
					current = &out
				}
