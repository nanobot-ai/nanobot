...
func (s *Server) resourcesList(ctx context.Context, _ mcp.Message, _ mcp.ListResourcesRequest) (*mcp.ListResourcesResult, error) {
	...
	agents, err := s.listAgents(ctx, struct{}{})
	if err != nil {
		return nil, err
	}

	chats, err := s.listChats(ctx, struct{}{})
	if err != nil {
		return nil, err
	}

	files, err := s.listFiles(ctx, struct{}{})
	if err != nil {
		return nil, err
	}

	workflowsList, err := s.listWorkflows(ctx, struct{}{})
	if err != nil {
		return nil, err
	}
...
func (s *Server) listFiles(ctx context.Context, _ struct{}) (*mcp.ListResourcesResult, error) {
	mcpSession := mcp.SessionFromContext(ctx)
...
func (s *Server) listWorkflows(ctx context.Context, _ struct{}) (*mcp.ListResourcesResult, error) {
...
func fileURI(path string) string {
	...
	return "file:///" + strings.TrimPrefix(filepath.ToSlash(absPath), "/")
}
