...
func toResponse(req *types.CompletionRequest, resp *schemas.BifrostResponsesResponse) (*types.CompletionResponse, error) {
	...
	for _, msg := range resp.Output {
		...
		case schemas.ResponsesMessageTypeMessage:
			if msg.Role != nil {
				result.Output.Role = string(*msg.Role)
			}
			if msg.Content != nil {
					...
					case schemas.ResponsesOutputMessageContentTypeRefusal:
						if msg.ID != nil {
							result.Output.Items = append(result.Output.Items, types.CompletionItem{
								...
								Content: &mcp.Content{
									Type: "text",
									Text: "REFUSAL: " + block.ResponsesOutputMessageContentRefusal.Refusal,
								},
							})
						}
					}
		...
		case schemas.ResponsesMessageTypeFunctionCall:
			if msg.ResponsesToolMessage != nil {
				...
				if msg.ResponsesToolMessage.Name != nil {
					item.ToolCall.Name = *msg.ResponsesToolMessage.Name
				}
				if msg.ResponsesToolMessage.Arguments != nil {
					item.ToolCall.Arguments = *msg.ResponsesToolMessage.Arguments
				}
				if msg.ResponsesToolMessage.CallID != nil {
					item.ToolCall.CallID = *msg.ResponsesToolMessage.CallID
				}
				result.Output.Items = append(result.Output.Items, item)
			}
		}
	}
